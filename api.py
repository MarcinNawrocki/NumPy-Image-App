...
def getBinaryzedImage(filename, threshold): #tested
    ...
    np_image = bs.readImage(filename)
    np_image_2D = bs.ensureGrayscale(np_image)
    np_final = bn.thresholdBinaryzation(np_image_2D, threshold)

    filename = filename + "1"
    bs.saveImage(np_final, filename)

def getOtsuBinaryzedImage(filename):#tested, read about images where is zeros in histogram
...
def getDilate(filename, struct_elem='rect', size=3): #tested
...
def getErode(filename, struct_elem='cross', size=3): #tested
...
def getOpenly(filename, struct_elem='rect', size=3): #Tested
    ...
    bs.saveImage(np_image_er, filename)
    np_final = bn.dilate(np_image_er, struct_elem, size)

    filename = filename + "1"
    bs.saveImage(np_final, filename)

def getClosely(filename, struct_elem='rect', size=3): #Tested
    """Execute openly(erode and dilate on the same image) morphological operation on image
    Function binaryzed image by OTSU method, so pass RGB or grayscale images possible.
    ...
    bs.saveImage(np_image_dil, filename)
    np_final = bn.erode(np_image_dil, struct_elem, size)

    filename = filename + "1"
    bs.saveImage(np_final, filename)

def filteringImage(filename, np_mask): #Tested
    """
...
def medianFiltergingImage(filename, struct_elem='rect', size=3):#Tested
    """
...
def gammaCorrection(filename, gamma): #Tested
    """
...
def addGaussianNoise(filename, std_dev=0.05, mean=0): #Grayscale looks good, test color
...
def addSaltPepperNoise(filename, propability = 0.05, saltPepperRatio = 0.5): #Tested
