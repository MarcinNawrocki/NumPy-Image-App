...
def getImageParameters(filename, save_source=True, defaultImagePath= "./public/python/images/"):
    ...
    try:
        ...
        print("Problem with handling JSON parameters file")
    finally:
        fp.close()
...
def getDilate(filename, struct_elem='rect', size=3,  number_of_inters=2): 
    ...
    np_image_2D, isConverted = bs.ensureGrayscale(np_image, info = True)
    start_image_number = 0
    #if image was converted to grayscale, we must save grayscale as interImage
    if isConverted:
...
def getOpening(filename, struct_elem='rect', size=3, number_of_inters=3): 
    """Execute opening(erode and dilate on the same image) morphological operation on image
    Function binaryzed image by OTSU method, so pass RGB or grayscale images possible.
...
def getClosing(filename, struct_elem='rect', size=3, number_of_inters=4): 
    """Execute closing(erode and dilate on the same image) morphological operation on image
    Function binaryzed image by OTSU method, so pass RGB or grayscale images possible.
...
def filteringImage(filename, np_mask_pom, number_of_inters=1): 
    ...
    Keyword argument:
        filename --  filename or path to image
        np_mask_pom -- mask matrix name(LP1-L4, HP1-HP4)
        number_of_inters -- number of inter images which should be generated by this operation
    Return:
        1 if operation was succesful.
    """
...
def removeFiles():   
    ...
    """
    defaultImagePath = "./public/python/images"
    for filename in os.listdir(defaultImagePath):
        file_path = os.path.join(defaultImagePath, filename)
        try:
